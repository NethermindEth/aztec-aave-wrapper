...
Privacy-preserving Aave lending from Aztec L2 via Wormhole bridge.

## Overview

Aztec Aave Wrapper enables users on Aztec L2 to deposit into Aave V3 on target chains (e.g., Arbitrum) while keeping their identity completely private. The system uses a three-layer architecture with cross-chain messaging to maintain privacy throughout the entire flow.

### Key Features

- **Privacy-Preserving**: User identity is never revealed on L1 or target chains
- **Cross-Chain**: Bridge assets from Aztec L2 to any Wormhole-supported chain
- **Aave V3 Integration**: Earn yield on deposited assets via Aave lending
- **Relayer Model**: Anyone can execute L1/target operations without knowing user identity

### MVP Constraints

- **USDC-only**: Single asset support for MVP
- **Full withdrawal only**: No partial withdrawals
- **Custodial model**: Target executor holds aTokens; user entitlement via private L2 receipts
...
│  │  - executeDeposit()              → Consumes L2 msg, bridges     │    │
│  │  - executeWithdraw()             → Sends withdrawal request     │    │
│  │  - receiveWormholeMessages()     → Receives confirmations       │    │
│  │  - completeWithdrawalTransfer()  → Completes token bridge       │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                    │                                     │
│                            Wormhole Bridge                               │
│                          (tokens + payload)                              │
...
│                          TARGET CHAIN                                    │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │  AaveExecutorTarget Contract (Solidity)                         │    │
│  │  - consumeAndExecuteDeposit()   → Supplies to Aave V3           │    │
│  │  - consumeAndExecuteWithdraw()  → Withdraws from Aave V3        │    │
│  │  - retryFailedOperation()       → Retries queued operations     │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                    │                                     │
│                              Aave V3 Pool                                │
└─────────────────────────────────────────────────────────────────────────┘
...
2. **Relayer Model**: L1 and target chain operations can be executed by anyone. The relayer doesn't need to know the user's identity - they just process the intent.
...
make build-target  # Target Solidity contracts
```
...
# Target chain node
docker compose logs anvil-target -f
...
│   │   ├── interfaces/        # IAztecOutbox, IWormhole, etc.
│   │   ├── types/             # Intent.sol, Confirmation.sol
│   │   └── mocks/             # MockWormhole contracts
│   └── foundry.toml
│
├── target/                    # Target executor contracts
│   ├── contracts/
│   │   ├── AaveExecutorTarget.sol
│   │   ├── interfaces/        # ILendingPool, IWormhole
│   │   ├── types/             # Intent.sol, FailedOperation.sol
│   │   ├── libraries/         # WormholeParser.sol
...
│       └── utils/             # Aztec helpers, Wormhole mock
│
├── scripts/
...
3. **Target: Relayer calls `consumeAndExecuteDeposit()`**
   - Verifies Wormhole VAA
   - Denormalizes amount (Wormhole uses 8 decimals)
   - Supplies tokens to Aave V3
   - Tracks per-intent shares
   - Sends confirmation back via Wormhole

4. **L1: Portal receives confirmation**
   - `receiveWormholeMessages()` processes confirmation
   - Sends L1→L2 message to Aztec inbox

5. **L2: User calls `finalize_deposit()`**
...
2. L1 portal sends withdrawal request to target
3. Target withdraws from Aave, bridges tokens back
4. L1 portal deposits tokens to Aztec token portal
5. User calls `finalize_withdraw()` to complete
...
| `ANVIL_TARGET_CHAIN_ID` | 31338 | Target chain ID |
...
- `TARGET_GAS_LIMIT`: 200,000

Target Executor (`AaveExecutorTarget.sol`):
- `WORMHOLE_DECIMALS`: 8 (Wormhole normalizes to 8 decimals)
...
```
intent_id = poseidon(caller, asset, amount, original_decimals, target_chain_id, deadline, salt)
```
...
- `target_chain_id`: Where the position exists
- `status`: Active, PendingWithdraw

### Per-Intent Share Tracking

The target executor tracks shares per `intent_id` (not per owner) to maintain privacy. This enables the anonymous pool model where the executor doesn't know which positions belong to which users.
