...
/**
 * @title PortalSimpleTest
 * @notice Unit tests for AztecAavePortalL1Simple contract
 * @dev Tests direct Aave integration without Wormhole bridging
 */
contract PortalSimpleTest is Test {
    AztecAavePortalL1Simple public portal;
    ...
    function setUp() public {
        ...
        // Deploy portal
        portal = new AztecAavePortalL1Simple(
            address(aztecOutbox),
            address(aztecInbox),
            address(tokenPortal),
            address(aavePool),
    ...
    function test_Constructor_RevertIf_ZeroOutbox() public {
        vm.expectRevert(AztecAavePortalL1Simple.ZeroAddress.selector);
        new AztecAavePortalL1Simple(
            address(0),
            address(aztecInbox),
            address(tokenPortal),
            address(aavePool),
    ...
    function test_executeDeposit_EmitsEvents() public {
        ...
        emit AztecAavePortalL1Simple.DepositExecuted(
            validDepositIntent.intentId, address(token), 1000e18, 1000e18
        );

        vm.expectEmit(true, false, false, true);
        emit AztecAavePortalL1Simple.DepositConfirmed(
    ...
    function test_executeDeposit_RevertIf_IntentAlreadyConsumed() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AztecAavePortalL1Simple.IntentAlreadyConsumed.selector, validDepositIntent.intentId
            )
        );
        vm.prank(relayer);
        portal.executeDeposit(validDepositIntent, l2BlockNumber, leafIndex, validSiblingPath);
    ...
    function test_executeDeposit_RevertIf_DeadlineTooSoon() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AztecAavePortalL1Simple.InvalidDeadline.selector, validDepositIntent.deadline
            )
        );
        vm.prank(relayer);
        portal.executeDeposit(validDepositIntent, l2BlockNumber, leafIndex, validSiblingPath);
    ...
    function test_executeWithdraw_EmitsEvents() public {
        ...
        vm.expectEmit(true, true, false, true);
        emit AztecAavePortalL1Simple.WithdrawExecuted(
            withdrawIntent.intentId, address(token), 1000e18
        );

        vm.expectEmit(true, false, false, true);
        emit AztecAavePortalL1Simple.WithdrawConfirmed(
    ...
    function test_executeWithdraw_RevertIf_NoSharesForIntent() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AztecAavePortalL1Simple.NoSharesForIntent.selector, validWithdrawIntent.intentId
            )
        );
        vm.prank(relayer);
        portal.executeWithdraw(
    ...
    function test_executeWithdraw_RevertIf_IntentAlreadyConsumed() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AztecAavePortalL1Simple.IntentAlreadyConsumed.selector, withdrawIntent.intentId
            )
        );
        vm.prank(relayer);
        portal.executeWithdraw(
